/*
Editorial : https://www.cnblogs.com/demian/p/6164613.html
*/
#include <cstdio>
#include <cmath>
#define LSon(x) ((x) << 1)
...
struct SegTree{
    ...
    int Query(int l, int r, int pos, int x) {
        if (l == r)
            return node[pos].degree;
        Push(pos);
        int m = l + r >> 1;
        if (x <= m)
            return Query(l, m, LSon(pos), x);
        else
            return Query(m + 1, r, RSon(pos), x);
    }
};
int n, c;
int s, a;
...
int main() {
    ...
    while (scanf("%d%d", &n, &c) != EOF) {
        ...
        for (int i = 0; i < c; i++) {
            scanf("%d%d", &s, &a);
            ///Because the title is counterclockwise, my calculation is clockwise. Add 180 degrees and see the back bar as still on the Y axis, so subtract the latter angle.
